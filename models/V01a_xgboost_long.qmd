---
title: "Xgboost Modeling Long. Classification with thirdile returns. V01a"
format: 
  html: 
    toc: true
    toc-depth: 2
...
stopping_grid <-grid_latin_hypercube(trees(range = c(500, 950)), 
...
yo_cost3 <- left_join(yo_precision, yo_recall) %>% left_join(., yo_soft_preci) %>% 
  ...
  mutate(soft_acc_rank = percent_rank(mean_soft_acc), recall_rank = percent_rank(mean_recall), 
         ...
         avg_rank =  0.2 * preci_rank + 0.4 * soft_preci_rank + 0.15 * soft_acc_rank + 0.25 * recall_rank)
