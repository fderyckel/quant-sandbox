---
title: "Xgboost Modeling Long 41 days. Classification with quintile returns put back into thirdile. V01c"
format: 
  html: 
    toc: true
    toc-depth: 2
...
yop <- glue('xgboost_v01c_', model_type, "_", ticker)
```
...
yo_cost3 <- left_join(yo_precision, yo_recall) %>% left_join(., yo_soft_preci) %>% 
  ...
  mutate(soft_acc_rank = percent_rank(mean_soft_acc), recall_rank = percent_rank(mean_recall), 
         ...
         avg_rank =  0.4 * preci_rank + 0.2 * soft_preci_rank + 0.25 * soft_acc_rank + 0.15 * recall_rank)
...
source(glue(the_path, "/models/functions/model_v01c.R"))

model <- 'v01c'
tickers = c('AAPL', 'AMD', 'AXP', 'CVX', 'FDX', 'HAL', 'SBUX') 
create_cor_df <- function(ticker) { 
  df0 <- model01c(ticker, num_days = 41)
  df <- df0 %>% drop_na()
...
write_csv(df1, glue(the_path, '/models/analysis/model_v01c_corr.csv'))
