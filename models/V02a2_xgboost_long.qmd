...
MAIN Difference from original Model 2 is that: 
* we have added a pre-processing steps (step_YeoJohnson)
* we have changed the measures for best models to account for more recall at the expense of some precision (recal = 0.3, precision = 0.55, acc = 0.15)
* time slices ... training on around 5 years and less number of samples but bigger validation set.  Time slices are non-cumulative. Ideally, validation set is > 30 and training set between 1000 and 1300. 
* decreased XV-Grid to 23.  We did not find clear patterns on the hyper-parameters tuning so far. 
* change the ATR denominator to 'close' instead of 'open'
...
yop <- glue('xgboost_v02a2_', num_days, "_", ticker)
```
...
df <- bind_cols(df, yo) |> mutate(atr11_open = atr11/close) |> select(-atr11)
yo <- tibble(atr17 = TTR::ATR(df[, 3:5], n = 17)[,2]) 
df <- bind_cols(df, yo) |> mutate(atr17_open = atr17/close) |> select(-atr17)
