...
yop <- glue('xgboost_v02aTRIAL_', model_type, "_", ticker)
```
...
# get the testing set (df2 is the df_model where we selected only the predictors)
df_testing <- df2 |> filter(date > '2023-06-30')

yo <- augment(model_base, new_data = df_testing)


library(vip)
final_wf |> fit(data = df_model) |> pull_workflow_fit() |> 
  vip(geom = 'point')
```
