...
yop <- glue('svm_v02a_short_', num_days, "_", ticker)
```
...
#stopping_grid <-grid_regular(cost(), rbf_sigma(), levels = 5)

wf_klab <- workflow() |> add_recipe(recipe_base) |> add_model(model_svm_klab) 
wf_liq <- workflow() |> add_recipe(recipe_base) |> add_model(model_svm_liq)  

doParallel::registerDoParallel()
svm_klab_resampl <- tune_grid(wf_klab, time_slices, 
                                grid = stopping_grid, 
...
df_pred <- svm_klab_resampl |> collect_predictions()
yo_soft_acc <- df_pred |> 
  mutate(soft_acc = case_when(.pred_class=="1" & ord_class == "1" ~ 1, 
  ...
  summarize(mean_soft_acc = mean(soft_acc), 
            cost = mean(cost), rbf_sigma = mean(rbf_sigma))
...
yo_metric <- yo_cost3 |> 
  select(.config, avg_rank, mean_preci, mean_soft_preci, mean_recall, 
         mean_soft_acc, cost, rbf_sigma) |> 
...
var_importance <- final_model %>% 
  set_engine("kernlab", importance = TRUE) %>% 
...
xgboost_final_fit2 <- read_rds('model_raw/svm_v02a_short_29d_CVX.rda')
xgboost_pred2 <- predict(xgboost_final_fit2, new_data = df_new_svm)
xgboost_pred_prob2 <- predict(xgboost_final_fit2, new_data = df_new_svm, type = 'prob')
df_pred_svm <- bind_cols(df_new_svm |> select(date, ord_class), 
                                 xgboost_pred2, xgboost_pred_prob2) |> 
  rename(ord_class_61d = ord_class)
