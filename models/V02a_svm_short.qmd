...
model_svm_liq <- svm_rbf(cost = tune(), rbf_sigma = tune()) |> 
...
svm_klab_resampl <- tune_grid(wf_klab, time_slices, 
                                ...
                                control = control_resamples(save_pred = TRUE, 
                                                            save_workflow = TRUE))
svm_liq_resampl <- tune_grid(wf_liq, time_slices, 
                                grid = stopping_grid, 
                                metrics = metric_set(accuracy, precision), 
                                control = control_resamples(save_pred = TRUE, 
                                                            save_workflow = TRUE))
...
yo_cost3 <- left_join(yo_precision, yo_recall) %>% left_join(., yo_soft_preci) %>% 
  ...
  mutate(soft_acc_rank = percent_rank(mean_soft_acc), 
         ...
         avg_rank =  avg_rank =  (0.25 * preci_rank) + (0.3 * soft_preci_rank) + 
...
yo_metric <- left_join(yo_precision, yo_soft_acc) |> left_join(yo_recall) |> 
  ...
  select(.config, avg_rank, cost, rbf_sigma) |> 
