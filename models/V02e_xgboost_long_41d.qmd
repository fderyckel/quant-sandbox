---
title: "Xgboost Modeling Long 41 days. Regression with percentile returns. V02e"
format: 
  html: 
    toc: true
    toc-depth: 2
...
yop <- glue('xgboost_v02e_', model_type, "_", ticker)
```
...
source(glue(the_path, "/models/functions/model_v2e.R"))

model <- 'version_2e'
tickers = c('AMD', 'AAPL', 'FDX', 'HAL', 'PYPL', 'CVX') 
create_cor_df <- function(ticker) { 
  df0 <- model_v2e(ticker)
  df <- df0 %>% drop_na()
...
write_csv(df1, 'correlations/model_v2e.csv')
