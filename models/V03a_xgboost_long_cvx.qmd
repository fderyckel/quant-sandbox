...
yop <- glue('xgboost_v03a_', {ticker}, '_', num_days, "_", ticker)
...
## Cross-validation on time-series slices 
...
## creating appropriate metrics for multi-class + collecting best parameters
```{r}
#| label: model_metrics
#| message: false
#| warning: false
...
xgboost_pred <- predict(xgboost_final_fit, new_data = df_new)
xgboost_pred_prob <- predict(xgboost_final_fit, new_data = df_new, type = 'prob')
df_pred_xgboost <- bind_cols(df_new |> select(date, ord_class), 
                                 xgboost_pred, xgboost_pred_prob)


xgboost_final_fit2 <- read_rds('model_raw/xgboost_v02a_29d_CVX.rda')
xgboost_pred2 <- predict(xgboost_final_fit2, new_data = df_new2)
xgboost_pred_prob2 <- predict(xgboost_final_fit, new_data = df_new, type = 'prob')
df_pred_xgboost2 <- bind_cols(df_new2 |> select(date, ord_class), 
