...
augment_df_fin <- function(df){
  df <- df |>   
    ...
    mutate(ret1d = log(adjusted / lag(adjusted, n = 1)), 
           ...
           ret_3M = log(close / lag(close, n = 61)), 
           ...
           roll_sd_ret1d_7M = sd_roll_7M(ret_1d), 
           roll_sd_ret3M_1Y = sd_roll_1Y(ret_3M),            
           ...
           roll_sd_volum200_1Y = sd_roll_1Y(volum200_perc), 
...
reduce_df <- function(df) { 
  df <- df |> 
    select(date, 
           ...
           roll_sd_ret21d_1Y, roll_sd_volum200_1Y, 
           ret_7w)
}
...
yop <- glue("xgboost_v8_2_", model_type, "_", ticker)
...
write_csv(var_importance, 
          paste0(the_path, "/models/model_vars_imp/", yop, ".csv"))
