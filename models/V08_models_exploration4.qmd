...
augment_df_fin <- function(df){
  df <- df |>   
    ...
    mutate(roc21 = log(adjusted / lag(adjusted, n = 21)), 
           ...
           roll_sd_volum200d_1Y = sd_roll_252(volum200_perc), 
...
yo_cost3 <- left_join(yo_precision, yo_recall) %>%  
  left_join(., yo_soft_preci) %>% 
...
write_csv(yo_metric, 
          paste0(the_path, "/models/model_metrics/xgboost_modelv8_explo4_", model_type, "_", ticker, ".csv"))
...
write_rds(model_base, 
          paste0(the_path, "/models/model_raw/xgboost_modelv8_explo4_", model_type, "_", ticker, ".rda"))
...
write_csv(var_importance, 
          paste0(the_path, "/models/model_vars_imp/xgboost_modelv8_explo4_", model_type, "_", ticker, ".csv"))
rm(df_prep)
