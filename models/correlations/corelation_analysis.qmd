...
yop <- glue(the_path, '/models/model_vars_imp/xgboost_modelv8_explo2_7wClassThirdile_')

df_VI_V82 <- tibble(ticker = c('AA', 'AAPL', 'AMD', 'GOOG')) |> 
  mutate(var_imp = map(ticker, get_var_imp)) |> 
  unnest(cols = c(var_imp)) |> 
  group_by(Variable) |> 
  summarise(mean_rank = mean(rank_order), 
...
yop <- glue(the_path, '/models/model_metrics/xgboost_modelv8_explo2_7wClassThirdile_')
model_name <- 'Model V8_2'

df_V82 <- tibble(ticker = c('AA', 'AAPL', 'AMD', 'GOOG')) |> 
...
yop <- glue(the_path, '/models/model_vars_imp/xgboost_modelv8_explo3_7wClassThirdile_')

df_VI_V83 <- tibble(ticker = c('AMD', 'CVX', 'HAL', 'PYPL', 'SBUX')) |> 
  mutate(var_imp = map(ticker, get_var_imp)) |> 
  unnest(cols = c(var_imp)) |> 
  group_by(Variable) |> 
  summarise(mean_rank = mean(rank_order), 
...
yop <- glue(the_path, '/models/model_metrics/xgboost_modelv8_explo3_7wClassThirdile_')
model_name <- 'Model V8_3'

df_V83 <- tibble(ticker = c('AMD', 'CVX', 'HAL', 'PYPL', 'SBUX')) |> 
  mutate(metrics = map2(ticker, model_name, get_metrics_V2)) |> 
  unnest(cols = c(metrics))
