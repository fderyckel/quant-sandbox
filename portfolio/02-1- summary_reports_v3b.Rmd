...
```{r download_data_1}
etf_tickers <- c("XLF", "XLI", "XLY", "XLV", "XME", "XLB", "XBI", "IYT", "HACK", "VOX", "ITB", "FXG", 'VCR', 'XLC', 'IYW', 'VGT', 
                 "SMH", "IPAY", "SPY", "RSP", "QQQ", "XLE", "VDE", "XOP", "GDXJ", "VAW", 'PBW', "EEM", 'DIA', 'GDX', 
...
pdf("charts_stocks.pdf", width=13, height=8, onefile = TRUE)
for (i in 1:nrow(df)){ 
  print(df$ticker[i])
...
df <- tibble(ticker = tickers, etf1 = etf1, etf2 = etf2) 
pdf("charts_tech.pdf", width=13, height=8, onefile = TRUE)
for (i in 1:nrow(df)){ 
  print(df$ticker[i])
  yo <- tibble(ticker = tickers[i], etf1 = etf1[i], etf2 = etf2[i]) %>% 
    mutate(raw_df = map(ticker, function(.x) conform_data(.x, "Daily", "fmpr")), 
...
df <- tibble(ticker = st) 
pdf("charts_portf.pdf", width=13, height=8, onefile = TRUE)
for (i in 1:nrow(df)){ 
  print(df$ticker[i])
...
df <- tibble(ticker = st) 
pdf("charts_stan.pdf", width=13, height=8, onefile = TRUE)
for (i in 1:nrow(df)){ 
  print(df$ticker[i])
  yo <- tibble(ticker = df$ticker[i]) %>% 
    mutate(raw_df = map(ticker, function(.x) conform_data(.x, "Daily", "fmpr")), 
...
df <- tibble(ticker = st) 
pdf("charts_foreign.pdf", width=13, height=8, onefile = TRUE)
for (i in 1:nrow(df)){ 
  print(df$ticker[i])
  yo <- tibble(ticker = df$ticker[i]) %>% 
    mutate(raw_df = map(ticker, function(.x) conform_data(.x, "Daily", "fmpr")), 
...
pdf("charts_markets.pdf", width=13, height=8, onefile = TRUE)
for (i in 1:nrow(df)){ 
  print(df$ticker[i])
  yo <- tibble(ticker = df$ticker[i]) %>% 
    mutate(raw_df = map(ticker, function(.x) conform_data(.x, "Daily", "fmpr")), 
...
```{r etf_returns}
return_ndays <- function(df, ndays = 12) { 
