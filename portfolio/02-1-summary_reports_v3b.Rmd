...
* On the conform data functions check for 0 or NA
...
```{r download_data_1}
etf_tickers <- c("XLF", "XLI", "XLY", "XLV", "XME", "XLB", "XBI", "IYT", "HACK", "VOX", 
                 "SMH", "IPAY", "SPY", "QQQ", "XLE", "VDE", "XOP", "GDXJ", "HYG", "EEM", "EWH",
                 "KWEB", "TLT", "DBB", "DBA", "GRU", "VAW", "ITB", "FXG", "IWD", "IWF")
...
tickers = c("AMD", "TSM", "BAH", "PYPL", "AXP", "JPM", 
            "GILD", "PKI", "BBWI", "SBUX", "MNST", "DIS", "PARA",
...
etf2 = c("SMH", "SMH", "HACK", "IPAY", "XLF", "XLF", 
         "XLV", "XLV", "XLY", "XLY", "FXG", "VOX", "VOX", 
         "IYT", "XLI", "XME", "XLE", "VDE", "VAW", "VAW")

df <- tibble(ticker = tickers, etf1 = etf1, etf2 = etf2) 
pdf(paste0("charts_", today(), ".pdf"), width=13, height=8, onefile = TRUE)
for (i in 1:nrow(df)){ 
  print(df$ticker[i])
...
```{r plot_data3}
st <- c("ABNB", "GOOG", "AAPL", "PYPL", "PARA", 'PICK', 'DBB', 'GDXJ', 'SILJ', 'COPX', 'KWEB', 'KGRN', 'KURE', 'KBA',  'EEM')
...
df <- tibble(ticker = st) 
pdf(paste0("charts_st_", today(), "2.pdf"), width=13, height=8, onefile = TRUE)
for (i in 1:nrow(df)){ 
  print(df$ticker[i])
  yo <- tibble(ticker = df$ticker[i]) %>% 
    mutate(raw_df = map(ticker, function(.x) conform_data(.x, "Daily", "fmpr")), 
           ext_df = map(raw_df, create_plot_fin_df))
  create_candlestick_chart(yo$ext_df[[1]], yo$ticker, start_date = today()-241, end_date = today())
  create_vol_candlestick_chart(yo$ext_df[[1]], yo$ticker, start_date = today()-241, end_date = today())
  Sys.sleep(2)
  }
dev.off()
```

Creating charts for etf. They are available for review as pdf in the portfolio folder. 
...
pdf(paste0("charts_markets", today(), ".pdf"), width=13, height=8, onefile = TRUE)
for (i in 1:nrow(df)){ 
  print(df$ticker[i])
  yo <- tibble(ticker = df$ticker[i]) %>% 
    mutate(raw_df = map(ticker, function(.x) conform_data(.x, "Daily", "fmpr")), 
...
return_ndays <- function(df, ndays = 21) { 
  yo <- df %>% mutate(returns = log(adjusted / lag(adjusted, n = ndays))) %>% 
    ...
    filter(index > today() - 2)
  return(yo)
}

etf_returns <- tibble(ticker = etf_tickers) %>% 
  mutate(raw_df = map(ticker, function(.x) conform_data(.x, "Daily", "fmpr")), 
         return_4w = map(raw_df, function(.x) return_ndays(.x))) %>% 
  select(-raw_df) %>% 
  unnest(col = "return_4w") %>% arrange(returns)
