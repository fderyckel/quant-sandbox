...
sell_countdown <- function(df) { 
  yo <- df |> 
    group_by(grp_sell_countdown = cumsum(TD_sell_setup == 9 | TD_buy_setup == 9)) |> 
    mutate(TD_sell_countdown = cumsum(close_greater_than_high_2earlier)) |> 
    ungroup()
  return(yo)
}

df <- conform_data('META') |> bullish_price_flip() |> bearish_price_flip() |> na.omit() |> 
  sell_setup() |> buy_setup() |> sell_countdown() |> 
  select(index, close, close_greater_than_high_2earlier, TD_buy_setup, TD_sell_setup, grp_sell_countdown, 
         TD_sell_countdown)
...
TD_sell_countdown <- function(ticker) {
  yo <- conform_data(ticker) |> bullish_price_flip() |> bearish_price_flip() |> 
    na.omit() |> sell_setup() |> buy_setup() |> sell_countdown() |> 
    select(index, close, close_greater_than_high_2earlier, TD_buy_setup, TD_sell_setup, 
           grp_sell_countdown, TD_sell_countdown)
  return(yo)
}
```
...
df2 <- conform_data('SQ') |> bullish_price_flip() |> bearish_price_flip() |> na.omit() |> 
  sell_setup() |> buy_setup() |> sell_countdown() |> 
  select(index, close, close_greater_than_high_2earlier, TD_buy_setup, TD_sell_setup, grp_sell_countdown, 
         TD_sell_countdown)

df2 <- conform_data('GOOG') |> bullish_td_price_flip() |> 
  bearish_td_price_flip() |> na.omit() |> 
  td_sell_setup() |> td_buy_setup() |> 
  select(index, close, 
         bullish_price_flip, bearish_price_flip, TD_sell_setup, TD_buy_setup)

df <- conform_data('ABNB') |> bullish_td_price_flip() |> 
  na.omit() |> td_sell_setup()
  td_buy_setup()
```
...
df <- tibble(tickers = c(xlk)) |> 
  distinct() |> arrange(tickers) |> 
  mutate(demark_indicator = map(tickers, TD_sell_countdown, .progress = T)) |>
  unnest(cols = c(demark_indicator)) |> 
  arrange(desc(index), desc(TD_sell_countdown))
```
