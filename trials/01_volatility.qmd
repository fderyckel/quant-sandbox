...
Question1: how to you meansure high volatility?  What would be your indicator for higher volatility? 
...
prepare_df <- function(ticker) { 
  pv <- read_csv(glue(the_path, '/data_stock_fmpr/', ticker, '.csv')) |> 
    ...
    mutate(ret1d = log(adj_close / lag(adj_close)), 
           ...
           actual_move = log(lead(adj_close, 35) / adj_close), 
           ret5d = log(adj_close / lag(adj_close, 5)), 
           sd5M_ret5d = roll_sd(ret5d, width = 103),
...
mean(pv$within_forecast, na.rm=T)
mean(pv$within_forecastB, na.rm=T)
```

```{r}
library(ggplot2)
ggplot(pv, aes(x = mult_sd3M_ret1d, y = actual_move)) + 
  geom_point()
```
