...
When doing cross-validation, we usually tend to have a training set, a validation set and then finally a testing set.  We need a testing set because whatever algorithm you have used has been fine-tuned to increase 'success' (however you measure it in your problem) on the validation set.  Hence, you have the best possible outcomes, but it doesn't mean it translates further to an 'accurate' forecasting (again, however ways you measure that in your problem).  This is why we save some observations (the latest one), to really see if our model have any predictive power. 
...
* assess: number of observations (time units) used for the associated testing sample 
...
* lag: if we want a lag (a certain amount of days) between the training and testing samples().  This is suggested to avoid any auto-correlation issues.  We are going to put a month of trading in between the training samples and testing samples

What took me a while to understand is the relationship between assess and lag.  So here is how I am currently understanding it.  
lag is based on assess.  It skips unit of times from the assess sample. And assess starts right after the initial samples. 
...
Problem can arise when the training set becomes then too small because the whole dataset is not big enough.  In this case, we'll then size down the skip time in order to have at least 500 training observations to work with.  

```{r}
#ticker <- 'SPY' 
ticker <- 'HACK'

df <- create_cv_data(ticker) #|> filter(index > '2009-09-01')
get_start_date <- function(x){ min(x$index) }
get_end_date <- function(x){ max(x$index) }
